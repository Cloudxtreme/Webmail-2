<?php
 ...
 class Evebit_Email {
      ...
      */
     public function mailConnect($host,$port,$user,$pass,$folder="INBOX",$ssl=false,$pop=false) {
         if($pop){
             $ssl = $pop.'/'.$ssl.'/novalidate-cert/notls';
         } 
         $this->_connect = imap_open("{"."$host:$port/imap/ssl"."}$folder",$user,$pass);
         if(!$this->_connect) {
...
$mailList = array();
 ...
 $connect = $mail->mailConnect('imap.qq.com',993,'774590565','zsk2aixgp','INBOX');
 if($connect) {
	 $totalCount =3;
	 for ($i = 3; $i > 0 ; $i --) {
